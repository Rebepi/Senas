...
export default function EntrenamientoPage({ initialLetter, maxSamples = 50 }: Props) {
  ...
  const handleTrain = async () => {
    if (!handData?.landmarks) return setStatusMessage("No hay datos de mano para entrenar");
    if (!isHandDetected) return setStatusMessage("No se detecta la mano");
  ...
  const handlePredict = async () => {
    if (!handData?.landmarks) return setStatusMessage("No hay datos de mano para predecir");
  ...
  return (
    <div className="p-6 w-full min-h-screen bg-gray-900 text-white flex flex-col items-center">
      ...
      <div className="flex gap-8 w-full max-w-[1200px] justify-center items-start flex-wrap">
        ...
        <div className="flex flex-col items-center gap-3 flex-1 min-w-[300px]">
          ...
          <p className="text-gray-400 text-sm text-center mb-2">
            Observa cómo debe posicionarse la mano.
          </p>
      ...
      <div className="flex gap-4 justify-center flex-wrap mt-8">
        <button
          onClick={handleTrain}
          disabled={!isHandDetected || sampleCount >= maxSamples || loading}
          className={`px-6 py-2 rounded-full font-bold transition-all duration-200 ${
            !isHandDetected || sampleCount >= maxSamples || loading
      ...
      {/* Mensaje de estado */}
      {statusMessage && (
        <p className="mt-2 text-sm text-green-400 font-medium">{statusMessage}</p>
      )}
